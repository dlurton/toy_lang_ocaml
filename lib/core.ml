...
(*
   Dumps the given environment to the console. This isn't used anywhere and can
   be deleted, but it is helpful when debugging.
*)
let dump_env env =
...
(* Extends the specified environment. *)
...
let eval e top_env : interp_result_t =
  ...
  in
  (* Catch any exception generated by inner_eval and return IR_error if any
     are encountered.  Otherwise, return IR_success. *)
  try
